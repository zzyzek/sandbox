...
(function() {
  ...
  libpoms.configStringify = function libpoms_stringify_config_json(poms_json) {
    ...
    if ("tileGroup" in poms_json){
      lines.push("\"tileGroup\":[");

      for (let ii=0; ii<poms_json.tileGroup.length; ii++) {
        let sfx = ( (ii==(poms_json.tileGroup.length-1)) ? "" : "," );
        ...
        multi_field.push( JSON.stringify(poms_json.tileGroup[ii]) + sfx );
      }
      if (multi_field.length > 0) {
        lines.push( multi_field.join(" ") );
      }
    ...
    if (typeof poms_json.flatTileset !== "undefined") {
      lines.push("\"flatTileset\":");

      lines.push( JSON.stringify(poms_json.flatTileset, undefined, 2) );
      lines.push(",");
    }
