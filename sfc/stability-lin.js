...
function mean_diff_data_nu(nu_0, nu_1) {
  ...
  for (let h_0=(wh[1]+dwh); h_0<H; h_0+=dwh) {
    ...
    mean_data.push( [w_0, h_0, w_1, h_1, S / p.length] );
...
function nu_nu_experiment() {
  ...
  for (let idx0=0; idx0 < nu_N; idx0++) {
    for (let idx1=0; idx1 < nu_N; idx1++) {
      ...
      let md = mean_diff_data_nu( nu_0, nu_1 );

      let fn = "data/nu_nu_" + idx0.toString() + "_" + idx1.toString() + ".gp";
      console.log("##", fn);
