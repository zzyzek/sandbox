...
function fig_elim2d() {
  ...
  */
  ...
  let dpq = [ p[0] - q[0], p[1] - q[1] ];
  let dpq_r90 = [ -dpq[1], dpq[0] ];
  let line_Npq = two.makeLine( q[0] + dpq_r90[0], q[1] + dpq_r90[1],
                               q[0] - dpq_r90[0], q[1] - dpq_r90[1] );
  line_Npq.dashes = [0,4,0];

  
  //----

  let _ldpq_r90 = Math.sqrt( (dpq_r90[0]*dpq_r90[0]) + (dpq_r90[1]*dpq_r90[1]) );
  let _dpqn_r90 = [ dpq_r90[0] / _ldpq_r90, dpq_r90[1] / _ldpq_r90 ];
  for (let i=-12; i<14; i++) {

    let _ds = 12;
    //let u = [ q[0] + dpq_r90[0]*i*_ds, q[1] + dpq_r90[1]*i*_ds ];
    let u = [ q[0] + _dpqn_r90[0]*i*_ds, q[1] + _dpqn_r90[1]*i*_ds ];

    let _t = theta + Math.PI/4;
    let _r = 30;
    let tl = [_r*Math.cos(_t), _r*Math.sin(_t)];
    //let _l = two.makeLine( q[0], q[1], q[0] + tl[0], q[1] + tl[1] );
    let _l = two.makeLine( u[0], u[1], u[0] + tl[0], u[1] + tl[1] );
  }
  ...
  /*
  let txt_q_shadow = two.makeText("q", q[0] + 10-1, q[1]+5+1, font_style_s);
  let _cs = "rgb(255,255,255)";
  //_cs = "rgb(0,0,0)";
  txt_q_shadow.fill = _cs;
  txt_q_shadow.stroke = _cs;
  txt_q_shadow.opacity = 0.5;
  //txt_q_shadow.noStroke();
  */

  let txt_q = two.makeText( "q", q[0] - 10, q[1] - 15, font_style);
