...
// also doesn't work?
// example below says it intersects in three triangels when it only should
// intersect two?
//
...
function gnuplot_cyl(cyl) {
  ...
  let Np = njs.clone( cyl.axis.direction );
  let p0 = njs.add( cyl.axis.origin, njs.mul( h2, Np ) );
  let p0n = njs.add( cyl.axis.origin, njs.mul( -h2, Np ) );

  p0 = njs.add( cyl.axis.origin, njs.mul( cyl.height, Np ) );
  p0n = njs.clone( cyl.axis.origin );
...
function gnuplot_tris(ofn, tris) {
  ...
  tris_lines.push( "#" + JSON.stringify(tris));

  for (let i=0; i<tris.length; i++) {

    let _f = 2* (1+Math.random()) ;
    _f = 1.0;
    ...
    for (let j=0; j<v.length; j++) {
      tris_lines.push( printf("%f %f %f", _f * v[j][0], _f * v[j][1], _f * v[j][2]) );
    }
    tris_lines.push( printf("%f %f %f\n\n", v[0][0] * _f, v[0][1] * _f, v[0][2] * _f) );
  }
...
cyl.axis.origin =  njs.sub( njs.mul(2, njs.random([3])), [1,1,1] );
cyl.axis.direction = njs.random([3]);
cyl.radius = Math.random();
cyl.height = Math.random();

box.extent = njs.random([3]);

// forced example...

cyl = {"axis":{"origin":[-0.956712146358091,-0.39435997847959614,-0.7274493703132572],"direction":[0.313921511660457,0.707433404517432,0.6332387090897785]},"radius":0.8187241945978961,"height":0.25417470402828135};
box = {"extent":[0.11400215067221942,0.6488406687732543,0.8801684909641376]};


cyl.axis.direction = njs.mul( 1 / njs.norm2(cyl.axis.direction), cyl.axis.direction );
