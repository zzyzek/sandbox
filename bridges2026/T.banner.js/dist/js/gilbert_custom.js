...
function _gilbert_xyz2d(x,y,z,w,h,d) {
  let _q = {"x": x, "y": y, "z": z};
  let _p = {"x": 0, "y": 0, "z": 0};
  let _a = {"x": w, "y": 0, "z": 0};
  let _b = {"x": 0, "y": h, "z": 0};
...
if (typeof module !== "undefined") {
  ...
  module.exports["d2xyz_r"] = gilbert_d2xyz_r;
  module.exports["xyz2d_r"] = gilbert_xyz2d_r;
  ...
  function _main(argv) {
    ...
    if (op == "xy2d") {
      ...
      let m = Math.floor(rho * n);
      let p = [];

      for (let i=0; i<m; i++) {
        p.push( [ Math.floor(Math.random() * w), Math.floor(Math.random() * h) ] );
      }

      for (let i=0; i<m; i++) {

        let p_idx = gilbert_xy2d(p[i][0],p[i][1],w,h);

        //console.log(p_idx, p, w,h);

        for (let _w=w; _w<=W; _w++) {
          let _h = Math.floor( h + ((H - h)*(_w-w)/(W-w)) );
          let _N = _w*_h;

          let q_idx = Math.floor( _N * p_idx / n );
          let q = gilbert_d2xy(q_idx, _w, _h);

          //console.log("# _w,_h:", _w, _h, "q_idx:", q_idx, "(p_idx:", p_idx,")");
      ...
      for (let i=0; i<m; i++) {
        console.log( Math.floor(Math.random()*w), Math.floor(Math.random()*h) );
      }
