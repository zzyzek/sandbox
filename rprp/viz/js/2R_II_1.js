...
function mathjax2twojs(_id,x,y,s,s_sub, opacity) {
...
function mkInnerGrid(pgn) {
  ...
  for (let y_idx=0; y_idx<Y.length; y_idx++) {
    for (let x_idx=0; x_idx<Y.length; x_idx++) {
...
function _2R_II_1_init() {
  ...
  mathjax2twojs( "alpha", alpha_pnt[0]-15, alpha_pnt[1]-5, 0.018 );
  ...
  let gp0_idx = rp_info.Gij[3][5];
  let gp1_idx = rp_info.Gij[2][5];
  let gp2_idx = rp_info.Gij[2][4];
  ...
  let _rect_l0_txt = mathjax2twojs( "rect_l0", rect_l0_xy[0]-4, rect_l0_xy[1]+20, 0.018 );
  let _rect_l1_txt = mathjax2twojs( "rect_l1", rect_l1_xy[0] + 8, rect_l1_xy[1] + 4, 0.018 );
  let _rect_l2_txt = mathjax2twojs( "rect_l2", rect_l2_xy[0]-4, rect_l2_xy[1] - 7, 0.018 );
  ...
  mathjax2twojs( "kappa", k_pnt[0] - 15, k_pnt[1] - 5, 0.018 );
