...
%  \end{frame}
  ...
  \begin{frame}[fragile]{Introduction}
    ...
    %where we fix a tile at a cell then keep
    %by removing tiles without valid neighbors

    %Basis for \textit{Constraint Propagation} algorithm:
    %\begin{itemize}
    %  \item fix a tile
    %  \item keep removing tiles without valid neighbors
    %\end{itemize}
